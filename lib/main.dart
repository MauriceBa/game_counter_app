...
class GamePlayer {
  ...
  GamePlayer({required this.name, this.score = 0, this.phase = 1});
...
class GameSession {
  ...
  factory GameSession.fromMap(Map<String, dynamic> map) {
    ...
    return GameSession(
      ...
      players: playersList.map((e) => GamePlayer.fromJson(Map<String, dynamic>.from(e))).toList(),
      notes: map['notes'],
    );
  }
}
...
class _ActiveGameScreenState extends State<ActiveGameScreen> {
  ...
  void initState() {
    super.initState();
    players = widget.playerNames.map((name) => GamePlayer(name: name)).toList();
  }
  ...
  @override
  Widget build(BuildContext context) {
    List<GamePlayer> displayPlayers = List.from(players);
